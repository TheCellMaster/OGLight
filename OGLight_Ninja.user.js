...
const oglRedirection = localStorage.getItem(UNIVERSE+'-ogl-redirect');
if(oglRedirection?.indexOf('https') > -1)
{
    ...
    localStorage.setItem(UNIVERSE+'-ogl-redirect', false);
...
const updateOGLBody = () =>
{
    document.body.dataset.minipics = localStorage.getItem(UNIVERSE+'-ogl_minipics') || false;
    document.body.dataset.menulayout = localStorage.getItem(UNIVERSE+'-ogl_menulayout') || 0;
    document.body.dataset.colorblind = localStorage.getItem(UNIVERSE+'-ogl_colorblind') || false;
    document.body.dataset.sidepanel = localStorage.getItem(UNIVERSE+'-ogl_sidepanelleft') || false;
}
...
class OGLight
{
    ...
    init(cache)
    {
        ...
        this.ptreKey = localStorage.getItem(UNIVERSE+'-ogl-ptreTK') || false;
        ...
        {
            window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch`;
            return;
        }
        ...
        $(document).on("ajaxSuccess", function(event, xhr, settings)
        {
            ...
            else if(settings.url.indexOf('action=miniFleet') >= 0)
            {
                ...
                if(status == 'fail')
                {
                    if(item.retry < 2)
                    {
                        item.uid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) { var r = Math.random() * 16 | 0, v = c == 'x' ? r : (r & 0x3 | 0x8); return v.toString(16); });
                        item.retry++;
...
class DomManager extends Manager
{
    ...
    loadBase(reload)
    {
        ...
        if(LFComponent) Util.addDom('a', { child:'genetics', href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=lfbonuses`, class:'ogl_lfBonusesLink tooltip', title:'Lifeforms Bonuses', parent:LFComponent });
    }
    ...
    updateTop()
    {
        ...
        if(wreckfield)
        {
            wreckfield.setAttribute('href', `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=facilities&openTech=36`);
        ...
        Object.values(this.ogl._dom.planet).forEach(planet =>
        {
            ...
            this.ogl._dom.redraw(div, fragment, () =>
            {
                if(!planet.classList.contains('tooltipLeft'))
                {
                    ...
                    {
                        const icon = Util.addDom('a', { class:'wreckFieldIcon tooltip js_hideTipOnMobile ogl_active ogl_wreckfieldIcon', parent:planet.parentNode, href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=facilities&cp=${id}&openTech=36`, child:'<span class="icon icon_wreck_field"></span>' });
...
class FetchManager extends Manager
{
    ...
    async fetchEmpire(type, callback, ignoreDelay)
    {
        ...
        return fetch(`${PROTOCOL}//${HOST}${window.location.pathname}?page=ajax&component=empire&ajax=1&planetType=${type}&asJson=1`,
        {
    ...
    async fetchLFBonuses(callback, ignoreDelay)
    {
        ...
        return fetch(`${PROTOCOL}//${HOST}${window.location.pathname}?page=ajax&component=lfbonuses`,
        {
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            signal: controller.signal
        })
    ...
    async fetchProductionQueue(callback, ignoreDelay)
    {
        ...
        return fetch(`${PROTOCOL}//${HOST}${window.location.pathname}?page=ajax&component=productionqueue&ajax=1`,
        {
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            signal: controller.signal
        })
...
class UIManager extends Manager
{
        ...
        if(!reloaded)
        {
            ...
            document.addEventListener('click', event =>
            {
                ...
                if(rawCoords)
                {
                    ...
                    {
                        const url = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=galaxy&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}`;
                ...
                || event.target.closest('.planetlink, .moonlink'))
                {
                    if(document.body.classList.contains('ogl_destinationPicker'))
                    {
                        ...
                        if(document.body.classList.contains('ogl_initHarvest'))
                        {
                            ...
                            window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${type}&oglmode=1&ogldestinationid=${id}&oglfirstsourceid=${firstSourceId}&ogldestinationtype=${type == 3 ? 'moon' : 'planet'}`;
                        }
                        else
                        {
    ...
    getPlayerTooltip(id)
    {
        ...
        const container = Util.addDom('div',
        {
            ...
            child:`
            <h1 class="${status}">${player.name} <a href="${PROTOCOL}//${HOST}${window.location.pathname}?page=highscore&site=${page}&searchRelId=${id}">#${player.score.globalRanking}</a></h1>
            <div class="ogl_grid">
        ...
        const writeIcon = Util.addDom('div', { child:'edit', class:'material-icons ogl_button', parent:container.querySelector('.ogl_actions'), onclick:() =>
        {
            ...
            {
                window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=chat&playerId=${player.uid}`;
            }
        }});
        ...
        // buddy
        Util.addDom('a', { child:'person_add', class:'material-icons ogl_button overlay', parent:container.querySelector('.ogl_actions'), href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=buddies&action=7&id=${player.uid}&ajax=1`, onclick:() =>
        {
            tippy.hideAll();
        }});
        ...
        {
            window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ignorelist&action=1&id=${player.uid}`;
        }});
    ...
    addSpyIcons(parent, coords, uniqueType, displayActivity)
    {
        ...
        if(uniqueType == 'planet' || !uniqueType)
        {
            const planetIcon = Util.addDom('div', 
            {
                ...
                onclick:e => window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=1`,
        ...
        {// onclick:e => this.ogl._fleet.addToSpyQueue(6, coords[0], coords[1], coords[2], 3)
            const moonIcon = this.ogl.db.pdb[`${coords[0]}:${coords[1]}:${coords[2]}`]?.mid > 0 ? Util.addDom('div', 
            {
                ...
                child:'bedtime',
                parent:parent,
                onclick:e => window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=3`
...
class TopbarManager extends Manager
{
    load(reloaded)
    {
        ...
        {
            window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&oglmode=2`;
        }});
    }
        ...
        options.forEach(opt =>
        {
            ...
            else if(opt == 'showMenuResources')
            {
                ...
                const select = Util.addDom('select', { parent:label, class:'dropdownInitialized', onchange:() =>
                {
                    ...
                    localStorage.setItem(UNIVERSE+'-ogl_menulayout', this.ogl.db.options[opt]);
            ...
            else if(isBoolean && opt != 'sim' && opt != 'expeditionValue')
            {
                ...
                const input = Util.addDom('input', { type:'checkbox', parent:label, onclick:() =>
                {
                    ...
                    {
                        localStorage.setItem(UNIVERSE+'-ogl_minipics', this.ogl.db.options[opt]);
                        document.body.setAttribute('data-minipics', this.ogl.db.options[opt]);
                    }
                    else if(opt == 'colorblindMode')
                    {
                        localStorage.setItem(UNIVERSE+'-ogl_colorblind', this.ogl.db.options[opt]);
                    ...
                    {
                        localStorage.setItem(UNIVERSE+'-ogl_sidepanelleft', this.ogl.db.options[opt]);
            ...
            else if(opt == 'ptreTeamKey')
            {
                ...
                const input = Util.addDom('input', { type:'password', placeholder:'TM-XXXX-XXXX-XXXX-XXXX', value:localStorage.getItem(UNIVERSE+'-ogl-ptreTK') || '', parent:label,
                oninput:() =>
                {
                    ...
                    {
                        input.classList.remove('ogl_danger');
                        localStorage.setItem(UNIVERSE+'-ogl-ptreTK', input.value);
    ...
    addPinnedItemToList(player, list)
    {
        ...
        const rankLink = Util.addDom('a', { class:'ogl_ranking', href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=highscore&site=${page}&searchRelId=${player.uid}`, child:'#'+player.score?.globalRanking || '?' });
    ...
    openPinnedDetail(id, update)
    {
        ...
        const updateSide = () =>
        {
            ...
            const writeIcon = Util.addDom('div', { child:'edit', class:'material-icons ogl_button', parent:actions, onclick:() =>
            {
                ...
                {
                    window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=chat&playerId=${id}`;
                }
            }});
            ...
            // buddy
            Util.addDom('a', { child:'person_add', class:'material-icons ogl_button overlay', parent:container.querySelector('.ogl_actions'), href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=buddies&action=7&id=${id}&ajax=1`, onclick:() =>
            {
                tippy.hideAll();
            }});
            ...
            {
                window.location.href = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ignorelist&action=1&id=${id}`;
            }});
...
class FleetManager extends Manager
{
    ...
    fleetSent(defaultRedirection)
    {
        ...
        {
            //localStorage.setItem(UNIVERSE+'-ogl-redirect', this.ogl.nextRedirection);
...
class GalaxyManager extends Manager
{
    ...
    updateRow(player, row, isOwn, coords)
    {
        ...
        Util.addDom('a', { class:'ogl_ranking', parent:row.querySelector('.cellPlayerName'), href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=highscore&site=${page}&searchRelId=${player.uid}`, child:'#'+player.score.globalRanking });
...
class MessageManager extends Manager
{
    ...
    loadSpyTable(isOutdated, forceClean)
    {
        ...
        list.forEach(message =>
        {
            ...
            const name =  Util.addDom('a', { child:message.playerName, parent:nameContainer, class:'overlay', href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=componentOnly&component=messagedetails&messageId=${message.id}` });
            const pin = Util.addDom('div', { parent:nameContainer });
            const loot = Util.addDom('a', { child:Util.formatToUnits(lootValue), parent:content, class:'ogl_textRight ogl_loot', href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1&am${this.ogl.db.options.defaultShip}=${requiredShips}&oglmode=4&oglmsg=${message.id}` });
            ...
            for(let i=1; i<7; i++)
            {
                ...
                this.ogl.fretShips.forEach(shipID =>
                {
                    ...
                    Util.addDom('a',
                    {
                        ...
                        href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1&am${shipID}=${shipsCount}&oglmode=4&oglLazy=true`
                    });
                });
            }
            ...
            const attackBtn = Util.addDom('a',
            {
                ...
                parent:actions,
                child:'swords',
                href:`${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${message.targetTypeID}&mission=1`
            });
...
class ShortcutManager extends Manager
{
    load()
    {
        ...
        this.add('showMenuResources', () =>
        {
            ...
            localStorage.setItem(UNIVERSE+'-ogl_menulayout', this.ogl.db.options.showMenuResources);
    ...
    getRedirectionLink(params)
    {
        ...
        let link = `${PROTOCOL}//${HOST}${window.location.pathname}?page=${params.page}`;
...
class TechManager extends Manager
{
        ...
        if(id == 36)
        {
            ...
            if(wreckfieldButton)
            {
                const checkWreckfield = tries =>
                {
                    ...
                    fetch(`${PROTOCOL}//${HOST}${window.location.pathname}?page=ajax&component=repairlayer&asJson=1`,
                    {
                        headers: { 'X-Requested-With': 'XMLHttpRequest' }
                    })
                    .then(response =>
    ...
    openTodolist(data, coords, id)
    {
        ...
        let url = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ingame&component=fleetdispatch&galaxy=${splitted[0]}&system=${splitted[1]}&position=${splitted[2]}&oglmode=3&targetid=${id}&type=${splitted[3]}`;
...
class EmpireManager extends Manager
{
    load()
    {
        ...
        unsafeWindow.jumpGateLink = `${PROTOCOL}//${HOST}${window.location.pathname}?page=ajax&component=jumpgate&overlay=1&ajax=1`;
